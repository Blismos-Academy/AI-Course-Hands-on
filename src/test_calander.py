import os.path
from datetime import datetime, timedelta

from google.auth.transport.requests import Request
from google.oauth2.credentials import Credentials
from google_auth_oauthlib.flow import InstalledAppFlow
from googleapiclient.discovery import build


SCOPES = [
    "https://www.googleapis.com/auth/calendar"
]


def get_calendar_service():

    creds = None

    if os.path.exists("token.json"):
        creds = Credentials.from_authorized_user_file(
            "token.json",
            SCOPES
        )

    if not creds or not creds.valid:

        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())

        else:
            flow = InstalledAppFlow.from_client_secrets_file(
                "credentials.json",
                SCOPES
            )

            creds = flow.run_local_server(port=0)

        with open("token.json", "w") as token:
            token.write(creds.to_json())

    service = build(
        "calendar",
        "v3",
        credentials=creds
    )

    return service


def create_reminder(service):

    # Tomorrow's date
    tomorrow = datetime.now() + timedelta(days=0)

    # Tomorrow at 5:00 PM
    start_time = tomorrow.replace(
        hour=16,
        minute=14,
        second=0,
        microsecond=0
    )

    # Event duration: 30 minutes
    end_time = start_time + timedelta(minutes=30)

    event = {
        "summary": "Dont forget to eat lunch",
        "description": "Reminder created by Weather AI Agent",
        "start": {
            "dateTime": start_time.isoformat(),
            "timeZone": "Asia/Kolkata",
        },
        "end": {
            "dateTime": end_time.isoformat(),
            "timeZone": "Asia/Kolkata",
        },
    }

    created_event = service.events().insert(
        calendarId="primary",
        body=event
    ).execute()

    print("Reminder created successfully!")
    print("Title:", created_event["summary"])
    print("Time:", start_time.strftime("%d-%m-%Y %I:%M %p"))
    print("Event ID:", created_event["id"])


def main():

    print("Starting Google Calendar test...")

    service = get_calendar_service()

    print("Google Calendar authentication successful!")

    create_reminder(service)


if __name__ == "__main__":
    main()