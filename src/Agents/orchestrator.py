...
class Orchestrator:
    ...
    def _planner(self, state: AgentState) -> dict:
        route = self.planner.invoke(state["question"], state.get("history", []))
        return {"route": route}
    ...
    def _tools(self, state: AgentState) -> dict:
        ...
        # Execute standard LangChain weather tools if needed
        if route in {"tools", "both"}:
            try:
                tool_res = self.tools.invoke(state["question"], state.get("history", []))
                results.extend(tool_res)
            except Exception as e:
                logger.error(f"Weather Tool Error: {e}")
                results.append(f"Weather Tool Error: {e}")

        # Execute MCP calendar integration if route requires mcp/calendar operations
        if route in {"mcp", "both"}:
            try:
                event = self._extract_event_details(state["question"])
                mcp_res = asyncio.run(
                    self.mcp_client.call_tool("create_calendar_event", event)
                )
    ...
    def _extract_event_details(self, question: str) -> dict:
        """Turn a natural-language request into the args create_calendar_event expects."""
        ...
        raw = str(self.llm.invoke([HumanMessage(content=prompt)]).content).strip()
        raw = raw.strip("`")
    ...
    def _response(self, state: AgentState) -> dict:
        context_parts = []
        if state.get("tool_results"):
            context_parts.append("Live context:\n" + "\n".join(state["tool_results"]))
